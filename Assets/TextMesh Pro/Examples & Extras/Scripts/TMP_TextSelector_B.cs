...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void RestoreCachedVertexAttributes(int index)
        {
            ...
            // Restore UV0S
            // UV S0
            // 1. Altera o tipo de 'src_uv0s' para Vector4[]
            // Isso assume que o membro '.uvs0' dentro da sua struct de cache 
            // (m_cachedMeshInfoVertexData) agora é do tipo Vector4[].
            UnityEngine.Vector4[] src_uv0s = m_cachedMeshInfoVertexData[materialIndex].uvs0;

            // 2. Altera o tipo de 'dst_uv0s' para Vector4[]
            // Isso corresponde ao novo tipo retornado pela API do TextMesh Pro no Unity 6.
            UnityEngine.Vector4[] dst_uv0s = m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0;

            // 3. O código de atribuição direta agora funciona sem erro de conversão (CS0029), 
            // pois ambos os arrays são Vector4[].
            dst_uv0s[vertexIndex + 0] = src_uv0s[vertexIndex + 0];
            dst_uv0s[vertexIndex + 1] = src_uv0s[vertexIndex + 1];
            dst_uv0s[vertexIndex + 2] = src_uv0s[vertexIndex + 2];
            dst_uv0s[vertexIndex + 3] = src_uv0s[vertexIndex + 3];

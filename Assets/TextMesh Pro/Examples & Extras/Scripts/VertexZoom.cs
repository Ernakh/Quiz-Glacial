        ...
        IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < characterCount; i++)
                {
                    ...
                    // Restore Source UVS which have been modified by the sorting
                    // 1. Altera o tipo de 'sourceUVs0' para Vector4[]
                    UnityEngine.Vector4[] sourceUVs0 = cachedMeshInfoVertexData[materialIndex].uvs0;

                    // 2. Altera o tipo de 'destinationUVs' para Vector4[]
                    UnityEngine.Vector4[] destinationUVs0 = textInfo.meshInfo[materialIndex].uvs0;

                    // 3. A atribuição agora é entre Vector4[] e Vector4[] (sem erro de conversão)
                    destinationUVs0[vertexIndex + 0] = sourceUVs0[vertexIndex + 0];
                    destinationUVs0[vertexIndex + 1] = sourceUVs0[vertexIndex + 1];
                    destinationUVs0[vertexIndex + 2] = sourceUVs0[vertexIndex + 2];
                    destinationUVs0[vertexIndex + 3] = sourceUVs0[vertexIndex + 3];
                ...
                for (int i = 0; i < textInfo.meshInfo.Length; i++)
                {
                    ...
                    // Cria um novo array Vector2[] a partir do Vector4[] (uvs0) para atribuir a mesh.uv
                    Vector2[] uv2s = new Vector2[textInfo.meshInfo[i].uvs0.Length];
                    for (int j = 0; j < uv2s.Length; j++)
                    {
                        // Copia apenas os componentes X e Y do Vector4 para o Vector2
                        uv2s[j] = new Vector2(textInfo.meshInfo[i].uvs0[j].x, textInfo.meshInfo[i].uvs0[j].y);
                    }
                    // Atribui o novo array Vector2[] ao mesh.uv
                    textInfo.meshInfo[i].mesh.uv = uv2s;
                ...
                // Antigo (problema):
                // textInfo.meshInfo[i].mesh.vertices = textInfo.meshInfo[i].vertices;
                // textInfo.meshInfo[i].mesh.uv = textInfo.meshInfo[i].uvs0; // <--- REMOVER

                // NOVO:
                // ... (apenas mantém a atualização de cores e a chamada final para UpdateGeometry)
                //textInfo.meshInfo[i].mesh.colors32 = textInfo.meshInfo[i].colors32;
                //m_TextComponent.UpdateGeometry(textInfo.meshInfo[i].mesh, i);
